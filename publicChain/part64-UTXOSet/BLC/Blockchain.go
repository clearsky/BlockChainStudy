...
func (blockchain *Blockchain) FindUTXOMap() map[string]*TXOutputs{
	...
	txInputsMap := make(map[string][]*TXInput)
	...
	for {
		...
		for i := len(block.Txs)-1; i>=0; i--{
			...
			// 遍历txInput，非创世区块才有input
			if !tx.IsCoinbaseTransaction(){
				for _, txInput := range tx.Vins{
					txHash := hex.EncodeToString(txInput.TxHash)
					txInputsMap[txHash] = append(txInputsMap[txHash], txInput)
				}
			}
			WorkOutLoop:
			for index, out := range tx.Vouts{
				txHash := hex.EncodeToString(tx.TxHash)
				txInputs := txInputsMap[txHash]
				if len(txInputs) > 0{
			...
			// 设置键值对
			if hasValid{
				utxoMap[hex.EncodeToString(tx.TxHash)] = txOutputs
			}
		}
...
func (blockchain *Blockchain) MineNewBlock(from []string, to []string, amount []string) {
	...
	for index, address := range from {
		...
		//奖励
		txReward := NewCoinbaseTransaction(address)
		txs = append(txs, tx)
		txs = append(txs, txReward)
	}
	...
	for i := len(txs)-1;i>=0;i--{
		if blockchain.VerifyTransaction(txs[i], txs) != true{
			fmt.Println("签名验证失败...")
			os.Exit(1)
		}
	}
